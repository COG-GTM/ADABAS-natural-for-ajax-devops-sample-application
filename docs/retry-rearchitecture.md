...
* `AdabasSim(wait_limit=N)` + `submit(session, operation)` — hold-queue
  ...
  issues ET or BT: the record is *granted* to the oldest waiter (it owns
  the hold before its callable is re-run, so nothing the restarted callable
  does before the blocked statement can lose it the record), the rest stay
  queued ahead of any newcomer (one submitted while that waiter runs parks
  behind them) and get their turn from its ET/BT. `wait_limit`
