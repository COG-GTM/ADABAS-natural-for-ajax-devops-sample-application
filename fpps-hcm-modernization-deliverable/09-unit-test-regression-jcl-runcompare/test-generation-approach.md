...
The complete mapping of every existing test to the rule it protects is generated, not typed, in `regression-suite-map.md` (149 tests, 33 rules, 11 emitted message codes with 3 coverage gaps).
...
flowchart LR
    ...
    MODEL --> BEH["Behavioural tests<br/>test_conew_booking · test_crlist_listing · test_concurrency · test_retry<br/>76 tests"]
...
| Step 2 | `python3 -m unittest discover -s tests -v` — runs all 149 tests (behavioural, source-conformance, evidence-drift, package-integrity) |
...
| Regression suite (149 tests) | CI step 2 | Yes | Rules in `regression-suite-map.md` |
