...
The complete mapping of every existing test to the rule it protects is generated, not typed, in `regression-suite-map.md` (117 tests, 29 rules, 11 emitted message codes with 3 coverage gaps).
...
| Step 2 | `python3 -m unittest discover -s tests -v` — runs all 117 tests (behavioural, source-conformance, evidence-drift, package-integrity) |
...
| Regression suite (117 tests) | CI step 2 | Yes | Rules in `regression-suite-map.md` |
