...
The complete mapping of every existing test to the rule it protects is generated, not typed, in `regression-suite-map.md` (169 tests, 33 rules, 11 emitted message codes with 3 coverage gaps).
...
flowchart LR
    ...
    MODEL --> BEH["Behavioural tests<br/>test_conew_booking · test_crlist_listing · test_concurrency · test_retry<br/>96 tests"]
...
The counts (22 / 96 / 47, plus 4 package-integrity tests) come from `regression-suite-map.md`, which the generator's `--check` mode ties to `unittest` discovery.
...
| Step 2 | `python3 -m unittest discover -s tests -v` — runs all 169 tests (behavioural, source-conformance, evidence-drift, package-integrity) |
...
| Regression suite (169 tests) | CI step 2 | Yes | Rules in `regression-suite-map.md` |
