...
The complete mapping of every existing test to the rule it protects is generated, not typed, in `regression-suite-map.md` (72 tests, 25 rules, 11 emitted message codes with 3 coverage gaps).
...
flowchart LR
    ...
    SRC -->|"analysed by"| DISP["Evidence drift tests<br/>tests/test_disposition_analysis.py<br/>11 tests"]
...
| Step 2 | `python3 -m unittest discover -s tests -v` — runs all 72 tests (behavioural, source-conformance, evidence-drift) |
...
| Regression suite (72 tests) | CI step 2 | Yes | Rules in `regression-suite-map.md` |
